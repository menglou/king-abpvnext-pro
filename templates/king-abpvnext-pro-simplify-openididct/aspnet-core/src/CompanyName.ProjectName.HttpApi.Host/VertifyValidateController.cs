...
using King.AbpVnextPro.Openiddict.Verification;
...
namespace CompanyName.ProjectName
{
    ...
    public class VertifyValidateController : AbpOpenIdDictControllerBase, ITokenExtensionGrant
    {
        ...
        protected IVerificationAppService _verificationAppService => LazyServiceProvider.LazyGetRequiredService<IVerificationAppService>();

        public string Name => VerifyCodeExtensionGrantConsts.GrantType;
        ...
        public virtual async Task<IActionResult> HandleAsync(ExtensionGrantContext context)
        {
            ...
            //验证验证码无效
            string vertifycode = context.Request.GetParameter("vericode").ToString();
            string vertifyId = context.Request.GetParameter("vericodeId").ToString();

            string result = await _verificationAppService.GetVerifyCodeMd5FromCache(vertifyId, vertifycode);
            ...
            else
            {
                var properties = new AuthenticationProperties(new Dictionary<string, string>
                {
                    ...
                    [OpenIddictServerAspNetCoreConstants.Properties.ErrorDescription] = result
                });
        ...
        private async Task ReplaceEmailToUsernameOfInputIfNeeds(OpenIddictRequest request)
        {
        ...
        private async Task<IActionResult> HandleTwoFactorLoginAsync(OpenIddictRequest request, IdentityUser user)
        {
        ...
        private async Task<IActionResult> SetSuccessResultAsync(OpenIddictRequest request, IdentityUser user)
        {
        ...
        private async Task<bool> IsTfaEnabledAsync(IdentityUser user)
        {
    ...
    public static class VerifyCodeExtensionGrantConsts
    {
        public const string GrantType = "vertifycode";
    }
}
