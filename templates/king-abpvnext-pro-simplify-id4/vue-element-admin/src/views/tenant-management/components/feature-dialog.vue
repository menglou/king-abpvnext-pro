  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getFeatures(this.featuresQuery).then((response) => {
        for (let i of response.groups) {
          this.features.push(...i.features);
        }
        this.features.map((feature) => {
          if (feature.valueType.name === "ToggleStringValueType") {
            this.$set(this.temp, feature.name, feature.value === "true");
          } else if (feature.valueType.name === "FreeTextStringValueType") {
            this.$set(this.temp, feature.name, feature.value);
          }
        });
      });
    ...
    updateData() {
      this.$refs["dataForm"].validate((valid) => {
        if (valid) {
          ...
          this.features.map((feature) => {
            if (feature.valueType.name === "ToggleStringValueType") {
              tempData.features.push({
                name: feature.name,
                value: String(!!this.temp[feature.name]),
              });
            } else if (feature.valueType.name === "FreeTextStringValueType") {
              tempData.features.push({
                name: feature.name,
          ...
          this.featuresQuery.providerKey=this.providerKey;
          this.featuresQuery.providerName=this.providerName
          debugger
          updateFeatures(this.featuresQuery, tempData).then(() => {
            this.dialogFormVisible = false;
            this.$notify({
              title: this.$i18n.t("IdentityServer['Success']"),
              message: this.$i18n.t("IdentityServer['SuccessMessage']"),
              type: "success",
              duration: 2000,
            });
          });
